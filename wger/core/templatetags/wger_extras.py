...
def get_current_settings(exercise, set_id):
    ...
    function in the template, and we are only interested on the settings 
...
def pagination(paginator, page):
    ...
    # For very long lists (e.g. the English ingredient with more than
    # 8000 items) we muck around here to remove the pages not immediately
    # 'around' the current one, otherwise we end up with a useless block with
    # 300 pages.
...
def trans_weight_unit(unit, user=None):
    ...
    :param user: the user object, needed to access the profile. If
                 this evaluates to False, metric is used
