...
class WorkoutLogUpdateView(WgerFormMixin, UpdateView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = \
            _(u'Edit log entry for %s') % self.object.exercise.name
...
def add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if dateform.is_valid() \
                and session_form.is_valid() and formset.is_valid():
        ...
        # update the current one or create a new one (this will be the most
        # usual case)
        if WorkoutSession.objects.filter(
